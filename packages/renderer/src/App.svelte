...
import TabbedListShell from './lib/layout/TabbedListShell.svelte';
import VolumeWorkspaceEntry from './lib/layout/VolumeWorkspaceEntry.svelte';
...
import { currentScreen, isTabsLayoutScreen, registerPrototype } from './stores/prototype';
import SubmenuNavigation from './SubmenuNavigation.svelte';

router.mode.memory();

// Generated by Cursor — titlebar prototype switcher (see .agents/skills/prototype-switcher)
registerPrototype<{ showTabsChrome: boolean; compactNav: boolean }>({
  name: 'Tabs & Flexible Layout',
  screens: [
    { value: 'tabs-layout', label: 'Version 1: Integrated' },
    { value: 'compact-nav', label: 'Version 2: Compact navigation' },
  ],
  overrides: {
    'tabs-layout': { showTabsChrome: true, compactNav: false },
    'compact-nav': { showTabsChrome: true, compactNav: true },
  },
});
  ...
  <main class="flex flex-col w-screen h-screen overflow-hidden">
    ...
    <div class="flex flex-row w-full h-full overflow-hidden">
      ...
      {#if $currentScreen === 'compact-nav'}
        <CompactNavigation meta={meta} exitSettingsCallback={(): void => router.goto(nonSettingsPage)} />
      {:else}
        <AppNavigation meta={meta} exitSettingsCallback={(): void => router.goto(nonSettingsPage)} />
      {/if}
      {#if meta.url.startsWith('/preferences') && $currentScreen !== 'compact-nav'}
      ...
      <div
        ...
        <!-- Legacy /workspace URL; list pages use TabbedListShell (permanent page tab + sections). -->
        <Route path="/workspace" breadcrumb="Workspace">
          <WorkspaceLayout />
        </Route>
        ...
        <Route path="/containers/*" breadcrumb="Containers" navigationHint="root" firstmatch>
          <Route path="/" breadcrumb="Containers" navigationHint="root">
            {#if isTabsLayoutScreen($currentScreen)}
              <TabbedListShell pageUrl="/containers" pageName="Containers" />
            {:else}
        ...
        <Route path="/images/*" breadcrumb="Images" navigationHint="root" firstmatch>
          <Route path="/" breadcrumb="Images" navigationHint="root">
            {#if isTabsLayoutScreen($currentScreen)}
              <TabbedListShell pageUrl="/images" pageName="Images" />
            {:else}
              <ImagesList />
            {/if}
          </Route>
        ...
        <Route path="/networks/*" breadcrumb="Networks" navigationHint="root" firstmatch>
          <Route path="/" breadcrumb="Networks" navigationHint="root">
            {#if isTabsLayoutScreen($currentScreen)}
              <TabbedListShell pageUrl="/networks" pageName="Networks" />
            {:else}
              <NetworksList />
            {/if}
          </Route>
          ...
          <Route path="/:name/:engineId/*" breadcrumb="Network Details" let:meta navigationHint="details">
            <NetworkWorkspaceEntry
              networkName={decodeURIComponent(meta.params.name)}
        ...
        <Route path="/pods" breadcrumb="Pods" navigationHint="root">
          {#if isTabsLayoutScreen($currentScreen)}
            <TabbedListShell pageUrl="/pods" pageName="Pods" />
          {:else}
            <PodsList />
          {/if}
        </Route>
        ...
        <Route path="/volumes/*" breadcrumb="Volumes" navigationHint="root" firstmatch>
          <Route path="/" breadcrumb="Volumes" navigationHint="root">
            {#if isTabsLayoutScreen($currentScreen)}
              <TabbedListShell pageUrl="/volumes" pageName="Volumes" />
            {:else}
              <VolumesList />
            {/if}
          </Route>
          ...
          <Route path="/:name/:engineId/*" breadcrumb="Volume Details" let:meta navigationHint="details">
            <VolumeWorkspaceEntry
              volumeName={decodeURI(meta.params.name)}
              engineId={decodeURI(meta.params.engineId)} />
          </Route>
        </Route>
        ...
        <Route path="/secrets/*" breadcrumb="Secrets" navigationHint="root" firstmatch>
          <Route path="/" breadcrumb="Secrets" navigationHint="root">
            {#if isTabsLayoutScreen($currentScreen)}
              <TabbedListShell pageUrl="/secrets" pageName="Secrets" />
            {:else}
              <SecretsList />
            {/if}
          </Route>
          ...
          <Route path="/:engineId/:secretId/*" breadcrumb="Secret Details" let:meta navigationHint="details">
            <SecretWorkspaceEntry
              secretId={decodeURIComponent(meta.params.secretId)}
              engineId={decodeURIComponent(meta.params.engineId)} />
          </Route>
        </Route>
